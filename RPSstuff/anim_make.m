...
switch computer
    case {'PCWIN','PCWIN64'}
        cmd=sprintf('!videomach /OpenFile=\\rps\\tmp\\%s*.png /SaveVideo=%s.flc /Start /Exit',anim_name,anim_name)
        
        eval(cmd);
        cmd=sprintf('!videomach /OpenFile=\\rps\\tmp\\%s*.png /SaveVideo=%s.avi /Start /Exit',anim_name,anim_name)
        eval(cmd);
    case 'GLNX86'   %my 32 bit Linux machine
