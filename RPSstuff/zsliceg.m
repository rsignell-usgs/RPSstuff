function varz=zsliceg(data,grid,depth)
% ZSLICEG: horizontal slice 3D data at fixed vertical level
%         using generic input structures (e.g returned by the NJ Toolbox)
% Usage: varz=zsliceg(data,grid,depth);
% Input: data = 3D array of data
%        grid  = grid structure containing 1D or 3D array of z values
%                (containing grid.z, which if 1d, will be expanded to 3d)
...
% [t,g]=nj_tslice(url,'temp',1);% grab 3d field of 'temp' at time step 1
% tz = zsliceg(t,g,-5);  % return temperature slice at 5 m depth
...
z=grid.z;
[~,ny,nx]=size(z);
if(nx==1 & ny==1),  % if z is a 1d array
    z3d(:,1,1)=z;
